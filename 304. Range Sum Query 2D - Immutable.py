...
class NumMatrix(object):
    ...
    def __init__(self, matrix):
        ...
        for _row in range(1, rows):
            self.dp[_row][0] = self.dp[_row - 1][0] + matrix[_row][0]

        for _row in range(1, rows):
            for _column in range(1, columns):
                self.dp[_row][_column] = self.dp[_row - 1][_column] + self.dp[_row][_column - 1] + matrix[_row][
                    _column] - self.dp[_row - 1][_column - 1]
